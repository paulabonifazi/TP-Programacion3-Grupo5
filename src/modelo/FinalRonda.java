...
/////////comision
	...
	public double calculoComision(ArrayList<ListAsignacionEmpleador>ListaDeContrataciones)
	{
		...
		for(int i=0; i< ListaDeContrataciones.size(); i++)//recorro la lista de contrataciones 
		{
			...
			for(int j=0; j <ListaEmpleadosContratados.size(); j++ )//recorro la lista de empelados
			{
				...
				sueldoPretendido = ListaEmpleadosContratados.get(j).getEmpleado.getTicket.getFB.getSueldoPretendido;
			
				if(ListaEmpleadosContratados.get(j).getEmpleado.getTicket.getFB.getTipoPuesto.equals("JUNIOR"))
					comision = sueldoPretendido * PorcentajeJunior;
				else
					if(ListaEmpleadosContratados.get(j).getEmpleado.getTicket.getFB.getTipoPuesto.equals("SENIOR"))
				...
				//PENSAR LA CONSIDERACION DE CUANTO SE RE RESTA!!!! 
				if(ListaEmpleadores.get(i).getRegistro.getDatos.puntajeUsuario > 0 && ListaEmpleadores.get(i).getRegistro.getDatos.puntajeUsuario <= 99)
					comision -= puntajeUsuario * 0.01;
				else
					if(ListaEmpleadores.get(i).getRegistro.getDatos.puntajeUsuario >= 100)
						comision = 0;
			...
			if(ListaEmpleadoresC.get(i).getRegistro.getDatos.getPersona.equais("PERSONAJURIDICA"))
			{
				if(ListaEmpleadoresC.get(i).getRegistro.getDatos.getRubro.equals("SALUD"))
					comisionEmpleador = sueldoPretendido * PorcentajePJSalud;
				else if(rubro.equals("COMERCIO LOCAL"))
			...
			if(ListaEmpleadoresC.get(i).getRegistro.getPuntajeUsuario != 0)
					comisionEmpleador -= ListaEmpleadoresC.get(i).getRegistro.getPuntajeUsuario * 0.01;
